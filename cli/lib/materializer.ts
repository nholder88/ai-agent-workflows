...
function determineDirectories(resolved: ResolvedContext): string[] {
  const dirs: Set<string> = new Set(['docs', '.cursor', '.github', '.github/workflows', '.github/skills']);
...
async function generateStandardsArtifacts(
  ...
  // Generate workspace skill stubs for recommended families
  if (variables.skillFamilies && variables.skillFamilies.length > 0) {
    filesCreated += generateSkillStubs(tempDir, variables);
  }

  return filesCreated;
}

function generateSkillStubs(tempDir: string, variables: TemplateVariables): number {
  let filesCreated = 0;
  
  for (const family of variables.skillFamilies || []) {
    const skillDir = path.join(tempDir, '.github', 'skills', family);
    if (!fs.existsSync(skillDir)) {
      fs.mkdirSync(skillDir, { recursive: true });
    }

    const stubContent = `# ${family.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} Skill

This is a workspace-local skill stub for the ${family} skill family.

## Purpose
Placeholder for ${family} implementation patterns and workflows specific to this project.

## Usage
This skill can be invoked by agents working on ${variables.projectName}.

## Implementation
Replace this stub with project-specific skill content as needed.

Generated by: ${variables.stackKey} template v${variables.templateVersion}
`;

    fs.writeFileSync(path.join(skillDir, 'SKILL.md'), stubContent, 'utf8');
    filesCreated++;
  }
